/*
 * ================================================================
 * DEBUG VARIANTE A: OHNE BMP280 (kein Luftdruck/Temperatur!)
 * Test: Läuft der Sensor-Stack ohne BMP280 stabil?
 * Falls JA → BMP280 ist das Problem!
 * ================================================================
 */
...
#include <WiFiUdp.h>
#include "MPU6050.h"
// #include <Adafruit_BMP280.h>   // <-- DEAKTIVIERT: BMP280
#include "AHT20.h"
...
IPAddress hostIP;
MPU6050 mpu;
// Adafruit_BMP280 bmp;          // <-- DEAKTIVIERT
AHT20 aht;
QMC5883P mag;
Madgwick filter;
...
void setup() {
    Serial.begin(115200);
    Serial.println("[DEBUG A] Start OHNE BMP280");
...
void loop() {
    if (dataReady) {
        ...
        // BMP280 deaktiviert → Druck und Temp bleiben 0
        static float p=0, t=0, h=0;
        static uint32_t lastEnv = 0;
        if(millis() - lastEnv > 100) {
            // p = bmp.readPressure();  // <-- DEAKTIVIERT
            aht.getSensor(&h, &t);
            lastEnv = millis();
        }
